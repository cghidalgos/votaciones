"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.JwtTokenModule = void 0;
const common_1 = require("@nestjs/common");
const jwt_service_1 = require("./jwt.service");
const jwt_abstract_1 = require("../../../domain/jwt/jwt.abstract");
const config_1 = require("../../../domain/config");
const jwt_config_service_1 = require("./jwt-config.service");
const strategies_1 = require("./strategies");
const passport_1 = require("@nestjs/passport");
const jwt_1 = require("@nestjs/jwt");
const domain_1 = require("../../../domain");
const implementations_1 = require("../../use-cases/implementations");
const bcrypt_module_1 = require("../bcrypt/bcrypt.module");
const postgres_db_services_1 = require("../postgres-db-services");
let JwtTokenModule = class JwtTokenModule {
};
exports.JwtTokenModule = JwtTokenModule;
exports.JwtTokenModule = JwtTokenModule = __decorate([
    (0, common_1.Module)({
        imports: [
            passport_1.PassportModule,
            bcrypt_module_1.BcryptModule,
            postgres_db_services_1.PostgresDbServicesModule,
            jwt_1.JwtModule.register({
                secret: process.env.JWT_SECRET,
                signOptions: { expiresIn: process.env.JWT_EXPIRATION_TIME },
            }),
        ],
        providers: [{
                provide: jwt_abstract_1.IJwtService,
                useClass: jwt_service_1.JwtTokenService
            }, {
                provide: config_1.IJwtConfigService,
                useClass: jwt_config_service_1.JwtConfigService
            },
            {
                provide: domain_1.IAuthUseCases,
                useClass: implementations_1.AuthUseCases
            },
            strategies_1.LocalStrategy,
            strategies_1.JwtStrategy
        ],
        exports: [domain_1.IAuthUseCases]
    })
], JwtTokenModule);
//# sourceMappingURL=jwt.module.js.map